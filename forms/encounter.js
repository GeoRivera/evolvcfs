(function() {
  ...
  function setCurrentDateTime(fieldName) {
    ...
    _setFormElement(fieldName, currDate);
    _setFormElement('time_' + fieldName, currTime);
  }
  ...
  function setElFromEval(xs) {
    for (var i = 0; i < xs.length; i++) {
      if (!_getFormElement(xs[i][0])) {
        if (_formElementExists(xs[i][0])) {
          _setFormElementFromData(xs[i][0], 'test_header_x', 'test_header_id', EVAL_ID, xs[i][1]);
        }
      }
    }
  }
  ...
  function setAgeIfBlank(fieldName) {
    ...
    if (!formAge) {
      if (currentAge) {
        _setFormElement(fieldName, currentAge);
      }
    }
  }
  ...
  function setClientUnder21YesNo(fieldName) {
    ...
    if (!answerOnForm) {
      if (currYesNo) {
        _setFormElement('udf_under21_TA', currYesNo);
      }
    }
  }
      ...
      if (gender_id) {
        switch (gender_id) {
          case 'AB51774C-2D91-401A-A604-3C8B4AC13119':
            result = 'EDD8F98B-D282-4145-997C-653E71F737D4';
            break;
          case '7B80D1E4-27A2-49E0-B7A3-697495F789C9':
            result = '725A7DEA-1BD3-4F71-ACF8-FE7A0026A672';
            break;
          case '2120501D-E8DC-4641-8D60-12C099E79F40':
            result = 'F8505020-CCFF-4504-816A-3F0D1E754692';
            break;
          ...
          default:
            result = 'C0DA946A-F0AD-4899-9C2D-C7123B6E1A36';
            break;
        }
        _setFormElement(fieldName, result);
      }
    }
  }
  ...
  function setZipAtEventDate(fieldName) {
    ...
    // Getting error when including date due to '<=' sign. Removing for now as the Form is run shortly enough after the Eval for the date to be irrelevant.
    // var condition = 'people_id = \'\'' + people_id + '\'\' AND is_deleted = 0 AND convert( DATE, address_date, 101 ) <= convert( DATE,\'\'' + event_date + '\'\', 101)';
    var condition = "people_id = ''" + people_id + "'' AND is_deleted = 0";

    var order_by = 'address_date DESC';

    var result = _getDataValue('address ', 'people_id', people_id, 'zip_code', condition, order_by);
      ...
      if (language_id) {
        switch (language_id) {
          case '785F9693-7DE5-400D-B51E-F2EB894DE76A':
            result = '781A58DE-5473-4A30-B9E1-18EF13DF3BF7';
            break;
          case '540DF61D-FD6F-4A0C-9EBE-58D4B27C0E37':
            result = '4AC644D8-D24A-4148-BBF8-251589719DF3';
            break;
          case '728E73AA-C4E7-49C2-ADB8-B7303455FACC':
            result = 'F22FA373-F1AD-4CBE-A915-25AB455B1147';
            break;
          case '28FA8610-B649-433D-A448-73C4486285AE':
            result = 'DF0C49C7-3A6C-4A50-85A6-4351DA9D50EA';
            break;
          case '0D554F80-B5E9-49FB-BD77-2CB89AEBC786':
            result = '8CE698D0-7C66-42E0-9248-4F51A18BFD19';
            break;
          case '1128F1F6-D927-4E4A-AFF3-BA6755A479EB':
            result = '000199F0-8E17-4699-91A9-700EC5947CA1';
            break;
          case 'A8CADB16-03DF-41BD-9C13-970FD2EE7914':
            result = 'D802201E-138F-40BA-96B2-74A430BD994F';
            break;
          case 'A4DCFB1C-9A3B-432A-93E5-6F9424BB5FB0':
            result = '8A5242EA-2D95-4AE0-B07C-86F6916F2555';
            break;
          case '5790D698-210D-4542-A2F2-5E6F280D9689':
            result = 'B1B6DE82-6C74-4626-A964-9097A4B14EED';
            break;
          case '888D9B90-EBAD-4EB0-A080-A317B528D274':
            result = 'BB83A7E4-1448-44B3-A7F7-B4CEB7F1D990';
            break;
          case '739C0CCA-CCAC-4DE8-A7E5-9AA4770AC5CD':
            result = '717FDACE-A6EF-4DC7-81FC-D3B1319317E0';
            break;
          case '9FA28B23-74E1-462E-8809-0453E9E4706E':
            result = 'FF5521A5-1BA0-48AE-9CB6-F651F8CECE3B';
            break;
          case '1742A1C9-4DBE-4E89-B126-5604EB8C2305':
            result = 'DE558F58-D8BB-47FA-9715-FC7FA74CB239';
            break;

          default:
            result = '63EAFC8E-1F3B-44BF-B8DD-F38E930C0FD4';
            break;
        }
        _setFormElement(fieldName, result);
      }
  ...
  var DISPO_ID = '762182AD-CE07-4CF3-8939-D22589CAAA68';

  var setElFromEval = createFormElementSetter(EVAL_ID);
  var setElFromDispo = createFormElementSetter(getFormId(DISPO_ID));

  !_getFormElement('time_actual_date') ? setCurrentDateTime('actual_date') : null;
  setDtFromDm('udf_evaldate_TA', 'event_log', 'event_log_id', EVAL_ID, 'actual_date');
...
//     // ,["udf_inpatientdispo_TA", "udf_inpatientdispo_TA"] // doesn't exist in current Dispo Form
//   ]);
  ...
  setDtFromDm('udf_datetimearrivedined_TA', 'test_header_x', 'test_header_id', EVAL_ID, 'udf_datetimearrivedined_TA');

  // Missing UDF_COMMUNITYBASEDINT
  setElFromEval([
