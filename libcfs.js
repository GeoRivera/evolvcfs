...
(function (window, document, undefined) {
  window.parent.$$ = {
     ...
     */
    formState: function () {
      var domArray = $('.formbody > .formgroup :not([id^=group_],[id^=img],[id^=dimg],[id^=caption_],[id$=_prompt],[id^=on_load_code])').filter(function (index) {
        return this.id !== '' && this.id.charCodeAt(0) !== 13
      }).map(function () {
        return ((this.id.charCodeAt(0) == 13) ? this.id.substring(this.id.lastIndexOf(this.id.charAt(0) + 1), this.id.length) : this.id) + ': ' + $(this).val() + $(this).text() + (($(this).attr('type') === 'checkbox') ? $(this).is(':checked').toString().toUpperCase() : '');
      });
      var compositeArray = [];
      var resultArray = [];
